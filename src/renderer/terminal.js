 ...
 */
async function restartTerminal(projectPath, options = {}) {
  if (multiTerminalUI) {
    // Set the project first, then create terminal
    multiTerminalUI.setCurrentProject(projectPath);
    return await multiTerminalUI.createTerminalForCurrentProject(options);
  }
  return null;
}
...
/**
 * Get currently active terminal state.
 */
function getActiveTerminalState() {
  if (!multiTerminalUI) return null;
  const manager = multiTerminalUI.getManager();
  if (!manager || typeof manager.getActiveTerminalState !== 'function') return null;
  return manager.getActiveTerminalState();
}
